...
void arg_utils::map_genotype_to_ARG_diploid(ARG &arg, const std::vector<int> &genotype, int site_id) {
    ...
    if (genotype.size() != n / 2) {
        throw std::invalid_argument(
                THROW_LINE("Size of diploid genotype must be exactly half the number of ARG leaves"));
    }
    ...
    for (int i = 0; i < genotype.size(); i++) {
        if (genotype[i] == 2) {
            ...
            throw std::runtime_error(
                    THROW_LINE("Diploid genotypes should be 0/1/2."));
        }
    }
    ...
    while (heterozygotes.num_values() > 0) {
        ...
        } else if (edge_0->parent->height - edge_0->child->height >
                   edge_1->parent->height - edge_1->child->height) {
            edge = edge_0;
            current_carriers = candidates_0;
        } else {
            edge = edge_1;
...
ARGNode *
arg_utils::most_recent_common_ancestor(const ARG &arg, const DescendantList &descendants, const arg_real_t position) {
    if (arg.roots.empty()) {
        throw std::runtime_error(THROW_LINE("Call populate_children_and_roots() first."));
    }
...
DescendantList arg_utils::populate_relate_scores_lazy(const ARG &arg, const ARGNode *node,
    ...
    const double node_penalty =
            static_cast<double>(intersect_nondesc_carriers + intersect_desc_noncarriers) / static_cast
                    <double>(n);
    ...
    const bool condition_2 =
            static_cast<double>(intersect_nondesc_noncarriers) / static_cast<double>(std::max(n_nondesc,
                                                                                              n_noncarriers)) > 0.7;
